	...
	def parse(self):
		...
		def innerParse(buf, outputVars):
			...
			for item in self.reVar.finditer(buf):
				print('set', buf[pos:item.start()], item.groups()[0])
